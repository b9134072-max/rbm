...
function handleGetMenuData(data) {
  try {
    ...
    // Parse data menu dari struktur yang sudah ada
    for (let i = 2; i < values.length; i++) {
      const row = values[i];
      
      // Rice Bowl (kolom B, C, D, E)
      if (row[1] && row[1].trim() !== '') {
        menuData.riceBowl.push({
          nama: row[1] || '',
          foto: row[2] || '',
          harga: row[3] || '',
          deskripsi: row[4] || ''
      ...
      // Minuman (kolom J, K, L, M)
      if (row[9] && row[9].trim() !== '') {
        menuData.minuman.push({
          nama: row[9] || '',
          foto: row[10] || '',
          harga: row[11] || '',
          deskripsi: row[12] || ''
      ...
      // Es Campur (kolom N, O, P, Q)
      if (row[13] && row[13].trim() !== '') {
        menuData.esCampur.push({
          nama: row[13] || '',
          foto: row[14] || '',
          harga: row[15] || '',
          deskripsi: row[16] || ''
      ...
      // Gorengan (kolom R, S, T, U)
      if (row[17] && row[17].trim() !== '') {
        menuData.gorengan.push({
          nama: row[17] || '',
          foto: row[18] || '',
          harga: row[19] || '',
          deskripsi: row[20] || ''
